...
describe('BlockDetection', () => {
  ...
  describe('Scoring Engine', () => {
    ...
    test('tier boundaries: 0.7 is blocked, 0.4 is suspected, below 0.4 is clear', () => {
      ...
      // Exactly 0.7 -> blocked
      const blocked = computeScore([{ name: 'a', weight: 0.7, details: '', source: 'full' }]);
      assert.strictEqual(blocked.blockStatus, 'blocked');

      // 0.5 -> suspected
      const suspected = computeScore([{ name: 'a', weight: 0.5, details: '', source: 'full' }]);
      assert.strictEqual(suspected.blockStatus, 'suspected');

      // 0.3 -> clear
      const clear = computeScore([{ name: 'a', weight: 0.3, details: '', source: 'full' }]);
  ...
  describe('Signal Detectors', () => {
    describe('HTTP Status', () => {
      test('403 returns weight 0.85', async () => {
        ...
        const signal = result.signals.find((s) => s.name.startsWith('http_status_'));
        assert.ok(signal);
      ...
      test('extra status codes are detected with weight 0.85', async () => {
        const detection = new BlockDetection({ extraStatusCodes: [418] }, logger);
        const result = await detection.analyzeFast(
          { url: () => 'https://example.com' },
          { status: () => 418, headers: () => ({}) },
        );
    ...
    describe('Response Headers', () => {
      test('cf-mitigated: challenge returns weight 0.9', async () => {
        ...
        const signal = result.signals.find((s) => s.name === 'waf_header_cf_mitigated');
        assert.ok(signal);
      ...
      test('server: cloudflare returns weight 0.1', async () => {
        ...
        const signal = result.signals.find((s) => s.name === 'waf_header_cloudflare');
        assert.ok(signal);
    ...
    describe('Challenge Selectors', () => {
      test('page with #challenge-form returns weight 0.8', async () => {
        ...
        const result = await detection.analyzeFull(mockPage, null);
        const signal = result.signals.find((s) => s.name === 'challenge_selector');
        assert.ok(signal);
        assert.strictEqual(signal!.weight, 0.8);
      });
    });
      ...
      test('short text (<50 chars) produces visible_text_short signal', async () => {
        ...
        const mockPage = {
          ...
          evaluate: async (fn: any, ...args: any[]) => {
            ...
            if (fnStr.includes('outerHTML')) return { htmlLength: 500, textLength: 10 };
            return 'Hi'; // Very short text
          },
        };

        const result = await detection.analyzeFull(mockPage, null);
        const signal = result.signals.find((s) => s.name === 'visible_text_short');
        assert.ok(signal);
      ...
      test("strong keyword 'captcha' on short page returns weight 0.6", async () => {
        ...
        const mockPage = {
          ...
          evaluate: async (fn: any, ...args: any[]) => {
            const fnStr = fn.toString();
            if (fnStr.includes('querySelector')) return null;
            if (fnStr.includes('outerHTML')) return { htmlLength: 500, textLength: 100 };
            return 'Please solve this captcha to continue';
          },
        };

        const result = await detection.analyzeFull(mockPage, null);
        const signal = result.signals.find((s) => s.name === 'visible_text_keyword_strong');
        assert.ok(signal);
        assert.strictEqual(signal!.weight, 0.6);
      });
      ...
      test("weak keyword 'blocked' with word boundary returns weight 0.15", async () => {
        ...
        const mockPage = {
          ...
          evaluate: async (fn: any, ...args: any[]) => {
            const fnStr = fn.toString();
            if (fnStr.includes('querySelector')) return null;
            if (fnStr.includes('outerHTML')) return { htmlLength: 500, textLength: 200 };
            // Long enough to avoid strong keyword detection (>= 500 chars)
            return 'You are blocked from accessing this resource. ' + 'a'.repeat(500);
          },
        };

        const result = await detection.analyzeFull(mockPage, null);
        const signal = result.signals.find((s) => s.name === 'visible_text_keyword_weak');
        assert.ok(signal);
    ...
    describe('Word Boundary Matching', () => {
      ...
      test("'cloudflare' matches 'Powered by Cloudflare'", () => {
        const regex = new RegExp('\\b' + escapeRegex('cloudflare') + '\\b', 'i');
      ...
      test('fires when html >= 1000 bytes and ratio < 0.03', async () => {
        ...
        const mockPage = {
          ...
          evaluate: async (fn: any) => {
            const fnStr = fn.toString();
            if (fnStr.includes('querySelector')) return null;
            if (fnStr.includes('outerHTML')) return { htmlLength: 2000, textLength: 10 }; // ratio = 0.005
      ...
      test('does not fire when html < 1000 bytes', async () => {
        ...
        const mockPage = {
          ...
          evaluate: async (fn: any) => {
            const fnStr = fn.toString();
            if (fnStr.includes('querySelector')) return null;
            if (fnStr.includes('outerHTML')) return { htmlLength: 500, textLength: 5 }; // Small page
            return 'a'.repeat(600);
          },
        };
      ...
      test('does not fire when ratio >= 0.03', async () => {
        ...
        const mockPage = {
          ...
          evaluate: async (fn: any) => {
            const fnStr = fn.toString();
            if (fnStr.includes('querySelector')) return null;
            if (fnStr.includes('outerHTML')) return { htmlLength: 2000, textLength: 100 }; // ratio = 0.05
            return 'a'.repeat(600);
          },
        };
    ...
    describe('Redirect Chain', () => {
      test('redirect through challenge domain returns weight 0.7', async () => {
        ...
        const signal = result.signals.find((s) => s.name === 'redirect_to_challenge');
        assert.ok(signal);
    ...
    describe('Meta Refresh', () => {
      test('meta refresh to challenge URL returns weight 0.65', async () => {
        ...
        const result = await detection.analyzeFull(mockPage, null);
        const signal = result.signals.find((s) => s.name === 'meta_refresh_challenge');
        assert.ok(signal);
  ...
  describe('Two-Pass Flow', () => {
    ...
    test('full pass merges signals from fast pass', async () => {
      ...
      const fullResult = await detection.analyzeFull(mockPage, mockResponse, fastResult);
    ...
    test('full pass without fast pass still runs fast detectors', async () => {
      ...
      const result = await detection.analyzeFull(mockPage, mockResponse);
      const statusSignal = result.signals.find((s) => s.name.startsWith('http_status_'));
    ...
    test('analyzeSpa runs content-based detectors only', async () => {
      ...
      const httpSignals = result.signals.filter((s) => s.name.startsWith('http_status_'));
      assert.strictEqual(httpSignals.length, 0);
      // Should have title keyword signal
      const titleSignal = result.signals.find((s) => s.name === 'title_keyword');
  ...
  describe('Edge Cases', () => {
    test('null response handled gracefully in analyzeFast', async () => {
      const detection = new BlockDetection({}, logger);
      const result = await detection.analyzeFast({ url: () => 'https://example.com' }, null);
      assert.ok(result);
      assert.strictEqual(result.blockStatus, 'clear');
      assert.strictEqual(result.score, 0);
    });
    ...
    test('null response handled gracefully in analyzeFull', async () => {
      ...
      const mockPage = {
        url: () => 'https://example.com',
        title: async () => 'Normal Page',
        evaluate: async () => 'Normal page content with enough text to pass all checks normally',
      };
    ...
    test('extracts hostname correctly from various URLs', async () => {
      ...
      for (const tc of testCases) {
        ...
        assert.strictEqual(result.hostname, tc.expected, `Failed for URL: ${tc.url}`);
      }
    });
  });
