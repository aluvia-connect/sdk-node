...
title: Fetch
sidebar_position: 4
description: Use Aluvia with Fetch (Node.js)
---
...
- In Node.js, the most reliable approach is to use **Undici** (Node’s HTTP client) and pass a **dispatcher** (for example, `ProxyAgent`) to `fetch`.

## Recommended (Node.js): use `AluviaClient` (local proxy) + Undici `ProxyAgent`
...
<Tabs groupId="fetch-proxy-source">
